...
`topic` 是要翻译或回答的文本，最长 30 个字；`mode` 可选值为 `翻译`、`回答`；`mood` 可选值为 `正常`、`差`、`极差`；`length` 可选值为 `精辟`（4–8 字）、`中等`（12–24 字）、`正常`（25–65 字）。`mode` 省略或无效时默认 `翻译`，其余可选项省略或无效时默认 `正常`。接口会对异常请求进行限流，并在生成结果不符合要求时自动换一种策略重试。
