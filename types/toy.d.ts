...
// 注意：SDK 只在 B站环境中挂载（脚本被拦截 / 端外打开时不存在），因此声明为
// 可选 —— 任何 `window.toy.xxx` 访问都必须先判空，否则类型检查会报错。
interface Window {
  /** Toy JS SDK 实例，由 toy-sdk.js 加载后挂载到全局；B站环境外可能不存在。 */
  toy?: ToySDK.Toy
}

/** Toy JS SDK 实例（等价于 `window.toy`，同样可能不存在，使用前需判空）。 */
declare const toy: ToySDK.Toy | undefined
